        ...
        if request.method =='POST':
            ...
            if form.is_valid:
                try:
                    ...
                    data["mesg"] ="error al crear producto"
    elif action=='upd':
...
def poblar_base_datos(request):
    ...
    Producto.objects.create(id_producto=1,nombre="Llanta x4",precio=199990,categoria=Categoria.objects.get(id_categoria=1))
    Producto.objects.create(id_producto=2,nombre="Neumatico",precio=38990,categoria=Categoria.objects.get(id_categoria=1))
    Producto.objects.create(id_producto=3,nombre="Servicio freno",precio=69990,categoria=Categoria.objects.get(id_categoria=1))
    return redirect(producto,action='ins',id='-1')
